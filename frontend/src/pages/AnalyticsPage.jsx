		...
		<div className='min-h-screen bg-gradient-to-b from-gray-900 via-green-900 to-emerald-900 text-white py-8'>
			<div className='container mx-auto px-4 max-w-6xl'>
				...
				>
					...
					<p className='text-gray-400'>Discover high-demand items with low supply - opportunities for sellers!</p>
				</motion.div>
				...
				>
					<div className='flex items-start'>
						<TrendingUp size={24} className='text-green-400 mr-3 mt-1 flex-shrink-0' />
						<div>
							<h3 className='text-xl font-bold mb-2 text-green-400'>Seller Opportunity</h3>
				...
				{analytics.length === 0 ? (
					...
					>
						...
						<p className='text-gray-400'>Analytics will appear once users start searching</p>
					</motion.div>
				) : (
					<motion.div
						initial={{ opacity: 0 }}
											...
											>
												<td className='px-6 py-4'>
													<div className='flex items-center'>
														<Search size={16} className='text-green-400 mr-2 flex-shrink-0' />
														<span className='font-medium capitalize'>{item._id}</span>
													</div>
												</td>
												<td className='px-6 py-4'>
													<span className='text-lg font-bold text-green-400'>{item.searchCount}</span>
												</td>
												<td className='px-6 py-4'>
				...
				{analytics.length > 0 && (
					<motion.div
						...
						className='mt-8 p-6 bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-xl border border-gray-700'
					>
							...
							{analytics.slice(0, 10).map((item, index) => {
								...
								return (
									<div key={item._id} className='flex items-center gap-4'>
										<div className='w-8 text-right text-sm text-gray-400'>#{index + 1}</div>
										<div className='flex-1'>
											<div className='flex items-center mb-1'>
												<span className='text-sm font-medium capitalize mr-3'>{item._id}</span>
												<span className='text-xs text-gray-400'>({item.searchCount} searches)</span>
											</div>
