    ...
    return (
      <div className='min-h-screen bg-gradient-to-b from-gray-900 via-blue-900 to-cyan-900 flex items-center justify-center'>
        <div className='text-center'>
          ...
          <p className='text-gray-400 mt-4'>Loading request details...</p>
        </div>
      </div>
    );
  }
    ...
    >
      <div className='max-w-6xl mx-auto'>
        ...
        <button
          ...
          className='flex items-center gap-2 text-gray-300 hover:text-white mb-6 transition duration-200'
        >
          <ArrowLeft size={20} />
          Back to Requests
        </button>
        ...
        {/* Header */}
        <div className='bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-xl p-6 border border-gray-700 mb-6'>
          <div className='flex items-start justify-between mb-4'>
            <div>
              <h1 className='text-3xl font-bold text-white capitalize mb-2'>{request.serviceType}</h1>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 text-sm text-gray-400'>
            ...
            >
              ...
              <div className='space-y-3 mb-4'>
                <div>
                  ...
                  <p className='text-white font-semibold'>{request.userId.name || 'N/A'}</p>
                </div>
                <div>
                  <p className='text-gray-400 text-sm'>Email</p>
                  <p className='text-white'>{request.userId.email || 'N/A'}</p>
                </div>
                <div>
                  <p className='text-gray-400 text-sm'>Phone</p>
                  <p className='text-white'>{request.userId.phone || request.contactInfo?.phone || 'N/A'}</p>
                </div>
                <div>
                  <p className='text-gray-400 text-sm'>Location</p>
                  <p className='text-white'>{request.userId.location || request.location || 'N/A'}</p>
                </div>
              </div>
          ...
          {request.assignedTechnician ? (
            ...
            >
              ...
              <div className='space-y-3 mb-4'>
                <div>
                  <p className='text-gray-400 text-sm'>Name</p>
                  <p className='text-white font-semibold'>{request.assignedTechnician.name || 'N/A'}</p>
                </div>
                <div>
                  <p className='text-gray-400 text-sm'>Email</p>
                  <p className='text-white'>{request.assignedTechnician.email || 'N/A'}</p>
                </div>
                <div>
                  <p className='text-gray-400 text-sm'>Phone</p>
                  <p className='text-white'>{request.assignedTechnician.phone || 'N/A'}</p>
                </div>
                <div>
                  <p className='text-gray-400 text-sm'>Location</p>
                  <p className='text-white'>{request.assignedTechnician.location || 'N/A'}</p>
                </div>
              </div>
              ...
              <div className='flex items-center justify-center h-full'>
                <div className='text-center'>
                  ...
                  {request.quotes && request.quotes.length > 0 && (
                    <p className='text-yellow-400 mt-2'>{request.quotes.length} quote(s) received</p>
                  )}
                </div>
              </div>
            </motion.div>
        ...
        {request.quotes && request.quotes.length > 0 && (
          <motion.div
            ...
            className='bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-xl p-6 border border-gray-700 mb-6'
          >
                  ...
                  <div className='flex items-start justify-between'>
                    <div className='flex-1'>
                      ...
                      {quote.additionalNotes && (
                        <p className='text-gray-300 text-sm'>{quote.additionalNotes}</p>
                      )}
                ...
                return (
                  <div className='space-y-4'>
                    ...
                    <div>
                      ...
                      <div className='flex items-center gap-2'>
                        <p className='text-white flex-1'>{contact.email}</p>
                        <button
                          onClick={() => handleCopyToClipboard(contact.email, 'Email')}
                          className='p-2 bg-gray-700 hover:bg-gray-600 rounded-lg transition duration-200'
                          title='Copy Email'
                        >
                          <Copy size={16} />
                        </button>
                    ...
                    {(contact.phone || request.contactInfo?.phone) && (
                      <div>
                        ...
                        <div className='flex items-center gap-2'>
                          <p className='text-white flex-1'>{contact.phone || request.contactInfo.phone}</p>
                          <button
