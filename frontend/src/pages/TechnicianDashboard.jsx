...
const TechnicianDashboard = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className='min-h-screen bg-gradient-to-b from-gray-900 via-blue-900 to-cyan-900 py-8 px-4'>
      <div className='max-w-7xl mx-auto'>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className='mb-8'>
          <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
            <div>
              ...
              <p className='text-gray-300'>Manage your repair jobs and service listings</p>
              {user.expertise && (
                <p className='text-cyan-400 text-sm mt-1'>Expertise: {user.expertise}</p>
              )}
            </div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8'>
          {statCards.map((stat, index) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className='bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-xl p-6 border border-gray-700'>
              ...
              <Link to={stat.link} className='text-cyan-400 hover:text-cyan-300 text-sm font-medium flex items-center gap-1'>View Details <ArrowRight size={14} /></Link>
            </motion.div>
          ))}
        </div>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8'>
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className='bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-xl p-6 border border-gray-700'>
            <div className='flex items-center justify-between mb-4'>
              <h2 className='text-2xl font-bold text-white flex items-center gap-2'><Wrench size={24} className='text-cyan-400' />Service Requests</h2>
              <Link to='/technician-requests' className='text-cyan-400 hover:text-cyan-300 text-sm font-medium flex items-center gap-1'>View All <ArrowRight size={14} /></Link>
            </div>
            {loading ? (<div className='text-center py-8 text-gray-400'>Loading...</div>) : requests.length === 0 ? (
              ...
              <div className='space-y-3'>
                {requests.slice(0, 5).map((request) => (
                  <Link key={request._id} to={`/technician-requests/${request._id}`} className='block p-4 bg-gray-700 bg-opacity-50 hover:bg-opacity-80 rounded-lg transition duration-200'>
                    <div className='flex items-start justify-between'>
                      <div className='flex-1'>
                        <h3 className='text-white font-semibold mb-1'>{request.serviceType}</h3>
                        <p className='text-gray-400 text-sm mb-2'>{request.location}</p>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }} className='bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-xl p-6 border border-gray-700'>
            <div className='flex items-center justify-between mb-4'>
              <h2 className='text-2xl font-bold text-white flex items-center gap-2'><Package size={24} className='text-green-400' />My Service Listings</h2>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className='bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-xl p-6 border border-gray-700 mb-8'>
          <div className='flex items-center justify-between mb-4'>
            <h2 className='text-2xl font-bold text-white'>Eco Status & Reputation</h2>
          ...
          <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
            ...
            <div className='p-4 bg-gray-700 bg-opacity-50 rounded-lg text-center'>
              ...
              <p className='text-2xl font-bold text-blue-400'>{user.trustScore || 80}%</p>
            </div>
            <div className='p-4 bg-gray-700 bg-opacity-50 rounded-lg text-center'>
