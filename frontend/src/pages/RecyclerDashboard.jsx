...
const RecyclerDashboard = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className='min-h-screen bg-gradient-to-b from-gray-900 via-green-900 to-teal-900 py-8 px-4'>
      <div className='max-w-7xl mx-auto'>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className='mb-8'>
          <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
            <div>
              ...
              <p className='text-gray-300'>Track your recycling submissions and environmental impact</p>
            </div>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8'>
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className='bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-xl p-6 border border-gray-700'>
            <div className='flex items-center justify-between mb-4'>
              <h2 className='text-2xl font-bold text-white flex items-center gap-2'><Recycle size={24} className='text-green-400' />Recent Submissions</h2>
              <Link to='/recycling' className='text-green-400 hover:text-green-300 text-sm font-medium flex items-center gap-1'>View All <ArrowRight size={14} /></Link>
            </div>
            {loading ? (<div className='text-center py-8 text-gray-400'>Loading...</div>) : submissions.length === 0 ? (
              <div className='text-center py-8'>
                <Recycle size={48} className='mx-auto text-gray-600 mb-3' />
                <p className='text-gray-400 mb-4'>No recycling submissions yet</p>
              ...
              <div className='space-y-3'>
                {submissions.slice(0, 5).map((submission) => (
                  <Link key={submission._id} to={`/recycling/${submission._id}`} className='block p-4 bg-gray-700 bg-opacity-50 hover:bg-opacity-80 rounded-lg transition duration-200'>
                    <div className='flex items-start justify-between'>
                      <div className='flex-1'>
                        <h3 className='text-white font-semibold mb-1'>{submission.itemType}</h3>
                        <p className='text-gray-400 text-sm mb-2'>{submission.location} • {submission.estimatedWeight}kg</p>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }} className='bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-xl p-6 border border-gray-700'>
            <div className='flex items-center justify-between mb-4'>
              <h2 className='text-2xl font-bold text-white flex items-center gap-2'><Package size={24} className='text-blue-400' />Recycled Parts Listings</h2>
              <Link to='/my-listings' className='text-blue-400 hover:text-blue-300 text-sm font-medium flex items-center gap-1'>View All <ArrowRight size={14} /></Link>
            </div>
            {myListings.length === 0 ? (
              <div className='text-center py-8'>
                  ...
                  <Link key={listing._id} to={`/listing/${listing._id}`} className='block p-4 bg-gray-700 bg-opacity-50 hover:bg-opacity-80 rounded-lg transition duration-200'>
                    <div className='flex items-start justify-between'>
                      <div className='flex-1'>
                        ...
                        <p className='text-gray-400 text-sm mb-2'>{listing.category} • ${listing.price}</p>
                        <div className='flex items-center gap-4 text-xs text-gray-500'>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className='bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-xl p-6 border border-gray-700 mb-8'>
          ...
          <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
            <div className='p-4 bg-gray-700 bg-opacity-50 rounded-lg text-center'>
              <p className='text-gray-400 text-sm mb-1'>Eco Points</p>
              <p className='text-2xl font-bold text-green-400'>{user.ecoPoints || 0}</p>
            </div>
            <div className='p-4 bg-gray-700 bg-opacity-50 rounded-lg text-center'>
              <p className='text-gray-400 text-sm mb-1'>Total Recycled</p>
              <p className='text-2xl font-bold text-blue-400'>{stats.totalWeight.toFixed(1)} kg</p>
            </div>
            <div className='p-4 bg-gray-700 bg-opacity-50 rounded-lg text-center'>
              <p className='text-gray-400 text-sm mb-1'>Eco Tier</p>
              <TierBadge tier={user.ecoTier} />
            </div>
            <div className='p-4 bg-gray-700 bg-opacity-50 rounded-lg text-center'>
