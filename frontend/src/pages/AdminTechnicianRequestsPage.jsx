    ...
    >
      <div className='max-w-7xl mx-auto'>
        ...
        <div className='mb-8'>
          ...
          <p className='text-gray-300'>Monitor and manage all service requests</p>
        </div>
        ...
        <div className='bg-gray-800 bg-opacity-80 backdrop-filter backdrop-blur-lg rounded-xl p-6 border border-gray-700 mb-6'>
          <div className='flex items-center justify-between mb-4'>
            ...
            <button
              ...
              className='flex items-center gap-2 px-6 py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition duration-200'
            >
              <Filter size={20} />
              Filters
            </button>
          ...
          {showFilters && (
            ...
            >
              <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mb-4'>
                <div>
                  <label className='block text-white text-sm font-semibold mb-2'>Service Type</label>
                  <select
                    name='serviceType'
        ...
        <div className='mb-4'>
          <p className='text-gray-300'>
            Showing <span className='text-cyan-400 font-semibold'>{technicianRequests.length}</span> request{technicianRequests.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        {isLoading ? (
          <div className='text-center py-16'>
            ...
            <p className='text-gray-400 mt-4'>Loading service requests...</p>
          </div>
          ...
          >
            ...
            <h3 className='text-2xl font-bold text-white mb-2'>No Service Requests Available</h3>
                      ...
                      {request.userId && (
                        <div className='bg-gray-700 bg-opacity-50 rounded-lg p-3'>
                          <div className='flex items-center gap-2 mb-1'>
                            ...
                            <span className='text-white font-semibold text-sm'>Sent By:</span>
                          </div>
                          <p className='text-gray-300 text-sm'>{request.userId.name || 'User'}</p>
                          <p className='text-gray-400 text-xs'>{request.userId.location || 'N/A'}</p>
                        </div>
                      )}
                      ...
                      {request.assignedTechnician ? (
                        <div className='bg-gray-700 bg-opacity-50 rounded-lg p-3'>
                          <div className='flex items-center gap-2 mb-1'>
                            ...
                            <span className='text-white font-semibold text-sm'>Assigned To:</span>
                          </div>
                          <p className='text-gray-300 text-sm'>{request.assignedTechnician.name || 'Technician'}</p>
                          <p className='text-gray-400 text-xs'>{request.assignedTechnician.location || 'N/A'}</p>
                        </div>
                      ) : (
                        request.quotes && request.quotes.length > 0 && (
                          <div className='bg-gray-700 bg-opacity-50 rounded-lg p-3'>
                            ...
                            <p className='text-gray-300 text-sm'>{request.quotes.length} quote(s)</p>
                          </div>
                        )
                      )}
                    </div>
